...
def main():
    ...
    for board_key, board_dict in boards.items():
        ...
        if board_dict['completion'] > slowest_board_completion:
            slowest_board_completion = board_dict['completion']
            slowest_board = board_key

    numbers_called = [number for position, number in call_order.items() if position <= slowest_board_completion]
    last_number_called = call_order[slowest_board_completion]
    losing_board_numbers = list(boards[slowest_board]['layout'].values())
    unmarked_numbers = list(set(losing_board_numbers) - set(numbers_called))
